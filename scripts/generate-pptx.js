...
async function main() {
  ...
  const spec = args.sample ? sampleSpec(args.sampleStyle) : JSON.parse(fs.readFileSync(specPath, 'utf8'));
  ...
  if (spec.__layoutDiversified && normalizedSpecPath) writeNormalizedSpec(spec, normalizedSpecPath);
...
async function buildDeck(spec, specDir, outPath) {
  ...
  pptx.author = spec.author || 'Guizang PPTXGenJS Skill';
...
function addChrome(slide, ctx, color, mode) {
  const left = ctx.slideSpec.chromeLeft || ctx.spec.title || 'Guizang PPT';
  const right = ctx.slideSpec.chromeRight || `${String(ctx.index + 1).padStart(2, '0')} / ${String(ctx.total).padStart(2, '0')}`;
...
function magazineRadial(slide, ctx, s) {
  ...
  items.forEach((item, i) => {
    ...
    slide.addText(item.body || item.desc || '', { x: tx, y: boxY + 0.34, w: 1.95, h: 0.38, fontFace: FONTS.sansZh, fontSize: 9.1, color: s.fg, transparency: 28, margin: 0, fit: 'shrink' });
  });
  addFoot(slide, ctx, s.fg, 'magazine');
}
