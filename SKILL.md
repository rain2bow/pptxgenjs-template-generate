...
- 不要忽略生成器警告。应修复 JSON、减少文本、拆分页、调整布局，或补充缺失媒体后重新生成。
...
2. 在编写 JSON 前，为目标样式生成文本容量指南：

   ```bash
   node scripts/generate-pptx.js --capacity-guide cmb --out outputs/cmb-capacity-guide.md
   ```

3. 编写结构化 JSON 规格，并确保内容处于推荐字段范围内。
4. 在最终生成前，将 JSON 规格转换为用户友好的 Markdown 大纲：
...
6. 如果警告提到文本容量、媒体槽位、被忽略字段、只有标题的条目、重复布局或布局不兼容，请更新 JSON 并重新生成。
7. 验证原生/可编辑结构和布局安全性：
...
- 占位符控制：只有在有意保留空占位符时，才将 `allowEmptyMediaSlots` 设为 `true`。
...
- `textWeave`、`contentSynthesis`、`denseText`：非对称文本卡片页，支持 1 到 6 个要点，适合长文本拆块。
- `article`、`sectionList`：在 CMB 下渲染为 briefing 变体，适合正文型内容，不要当成杂志式长文章排版。
- `textGrid`、`fourCards`：在 CMB 下渲染为 textWeave 变体，卡片数量会自适应。
...
- 如果空白图片占位符是有意保留的，请设置 `allowEmptyMediaSlots` 为 `true`。
- 如果已知用户图片数量，应选择槽位数量匹配的布局。

### 分点规则

- 需要编号分点时，在 JSON 中显式写入 `points`、`bullets` 或 `list` 数组。
- 数组长度大于 1 时，CMB 卡片正文使用 PPT 原生编号；只有 1 条时渲染为普通正文，不强制编号。
- 不要要求生成器把一段长正文自动切成多个分点；应在生成 JSON 前由模型按语义拆分。

## 文本容量

为演示文稿编写 JSON 前，始终运行 `--capacity-guide`。容量范围是基于 Microsoft YaHei / Times New Roman 以及 36、28、16、14、12 pt 字号层级校准的。
...
如果任一检查发出警告，应缩短相关 JSON 字段、拆分内容、扩大布局空间或选择其他布局。生成器只会发出警告；不会静默重写或截断用户文本。
