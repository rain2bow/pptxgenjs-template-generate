...
此技能把原 HTML 横向翻页 PPT 复刻为 **pptxgenjs 原生生成 `.pptx`** 的工作流。保留视觉特点，移除 WebGL、翻页、Motion One 等浏览器动效；所有页面必须用 PowerPoint 原生文本框、形状、图片、线条和表格结构搭建，便于后续在 PowerPoint/WPS/Keynote 中编辑。

**硬约束**：不得用浏览器截图、HTML 转图片、PDF 转图片、整页背景图等方式伪装成 PPTX。只有用户明确放弃可编辑性并单独要求“截图版 PPTX”时，才可以另走转换路线；本技能默认永远是 pptxgenjs 纯生成。

**视觉一致性的定义**：PPTX 与原 HTML 技能保持同一视觉系统、版式语法和层级关系，包括主题色 token、字体分工、字号比例、网格/留白、图片槽位、页眉页脚、卡片/线条规则。不要承诺浏览器像素级完全一致，因为 PowerPoint 与浏览器的字体渲染、CSS 布局、WebGL、混合模式和抗锯齿不同，pptxgenjs 无法控制到像素级。

来源识别：本技能改造自 `guizang-ppt-skill`。来源、作者、赞助信息只用于识别技能，不要写入生成的 PPTX 页面、封面或配图。
...
- 主题色只能从 `references/themes.md` 的 5 套预设中选，不接受任意 hex 混搭。

### 风格 B · 瑞士国际主义

- 全程无衬线：`Inter` / `Helvetica Neue` / `Noto Sans SC` / `Microsoft YaHei UI`。
- 只用一个高饱和 accent：IKB 蓝、柠檬黄、柠檬绿、安全橙，见 `references/themes-swiss.md`。
...
2. 确认受众、场景、页数、素材、图片/截图需求、主题色和硬约束。信息足够时可以合理假设并在交付说明里写明。
3. 把内容整理成结构化 JSON 规格，优先使用 `scripts/generate-pptx.js` 已支持的版式。不得从 HTML 结果截图生成 PPTX。
4. 调用脚本生成 `.pptx`：
...
```bash
cd guizang-pptxgenjs-ppt-skill
npm install
```
...
风格派生模式仍使用 `import-pptx-template.js`，但生成的 spec 会额外包含：
- `themeTokens`: 从原 PPTX 无文本装饰形状中提取的主色/辅色 token。
- `slides[].templateDecorations`: 从原 PPTX 中提取的大面积无文本装饰形状，生成器会在背景之后、正文之前重画为低透明 PPT 原生形状。

该模式适合“参考原模板风格，但允许内容区重排、插入图表/卡片/图片槽”的场景。它会保留品牌颜色、局部装饰和视觉气质，但不承诺母版、动画、复杂组合形状或所有像素级位置完全继承。
...
`validate-pptx-native.js` 用于确认不是整页截图/整页图片伪装成 PPTX；`validate-pptx-layout.js` 用于检查明显的文本覆盖、元素冲突和底部安全区问题。校验通过后仍应打开 PPTX 做人工视觉核对。
...
仅在这些情况下读取 `references/`：
- 需要严格追溯原 HTML 视觉语言、主题色、Swiss layout lock 或 checklist。
- 生成结果视觉明显偏离，需要查参考规则。

普通内容生成、改文案、换主题、换版式、插图表、插 icon，不需要读取 `references/` 或 `scripts/*.js`。
## JSON Spec
...
- 移除动效：不写 PowerPoint 动画，不依赖 HTML、CSS、WebGL、浏览器截图或导出。
- 原 HTML 的 WebGL 背景只能用 pptxgenjs 原生几何纹理近似复刻：风格 A 用低透明等高线/弧线/纸感纹理；风格 B 用发丝网格、点阵或 ASCII 感字符矩阵。不能插入 HTML 渲染背景截图。
...
- 底部页码、页脚、来源信息不要遮挡主体，至少保留 0.35 英寸安全区。

## 资源导览

- scripts/generate-pptx.js: pptxgenjs 生成器，内置主题、版式函数和样例 spec；READABILITY.minFontSize 控制普通文本最小可读字号。
- scripts/import-pptx-template.js: 用户上传 PPTX 的结构化迁移和风格派生器，抽取文本/图片/表格/图表、主色和无文本装饰形状，推断为本技能 JSON spec，并可选直接生成新 PPTX。
...
- `references/layouts-swiss.md` 与 `references/swiss-layout-lock.md`: 原 HTML 风格 B 的视觉参考。
- `references/checklist.md`: 视觉自检清单。生成 PPTX 后仍应检查其中与字体、图片、网格、主题色相关的问题。

## 质量检查

交付前至少做这些检查：

1. `node scripts/generate-pptx.js --sample --out outputs/sample-guizang.pptx` 能成功运行。
...
6. 运行 `node scripts/validate-pptx-native.js path/to/deck.pptx`，确认不是截图/整页图片转换。
7. 运行 `node scripts/validate-pptx-layout.js path/to/deck.pptx`，检查文字是否被后绘制的色块/图片覆盖、文本框是否冲突、是否进入底部安全区。
8. 打开 PPTX 做视觉核对：每页必须能追溯到原 HTML 技能的一个布局语法或 Swiss Sxx 语法。若某页看起来像普通 PowerPoint 模板，或出现文字被色块覆盖、元素互相压住，必须回退重做。
